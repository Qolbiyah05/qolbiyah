...
$cek_pinjaman_result = $koneksi->query($cek_pinjaman_query);
...
$buku_result = $koneksi->query($buku_query);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && empty($pesan)) {
    ...
    if ($koneksi->query($query) === TRUE) {
        ...
        $pesan = "<div class='alert alert-danger text-center'>Terjadi kesalahan: " . $koneksi->error . "</div>";
    }
}
?>
...
<body>
                        ...
                        <select name="buku_id" class="form-select" required>
                            <?php while ($buku = $buku_result->fetch_assoc()) { ?>
                                <option value="<?= $buku['id'] ?>">
                                    <?= htmlspecialchars($buku['judul']) ?> - (Tersedia: <?= $buku['tersedia'] ?>)
                                </option>
                            <?php } ?>
                        </select>
                    </div>
    ...
    <!-- JavaScript untuk menyembunyikan notifikasi otomatis -->
    <script>
        setTimeout(function() {
